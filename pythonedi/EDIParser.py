...
class EDIParser(object):
    ...
    def parse(self, data):
        ...
        if self.edi_format is None:
            raise NotImplementedError(
                "EDI format autodetection not built yet. Please specify an EDI format."
            )
        ...
        while len(edi_segments) > 0:
            ...
            for seg_format in self.edi_format:
                ...
                if seg_format["id"] == segment_name and seg_format["max_uses"] == 1:
                    ...
                    segment_obj, edi_segments = self.parse_repeating_segment(
                        edi_segments, seg_format
                    )
                    break
                elif seg_format["id"] == "L_" + segment_name:
                    # Found a loop
                    segment_name = seg_format["id"]
                    segment_obj, edi_segments = self.parse_loop(
                        edi_segments, seg_format
                    )
                    break
    ...
    def parse_segment(self, segment, segment_format):
        ...
        if fields[0] != segment_format["id"]:
            raise TypeError(
                "Segment type {} does not match provided segment format {}".format(
                    fields[0], segment_format["id"]
                )
            )
        ...
        to_return = {}
        for field, element in zip(
            fields[1:], segment_format["elements"]
        ):  # Skip the segment name field
            key = element["id"]
